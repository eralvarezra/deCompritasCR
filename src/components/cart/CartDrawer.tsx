                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[#a04792] font-semibold text-sm mt-0.5">
                ...
                {preOrderItems.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2 px-1">
                      ...
                      <span className="text-sm font-medium text-pink-800">Pre-pedido ({preOrderItems.length}) ~{settings.preorder_delivery_time}</span>
                    </div>
                    ...
                    <div className="bg-pink-50 border border-pink-200 rounded-xl p-3 mb-3">
                      <div className="space-y-1 text-sm">
                        <div className="flex justify-between text-gray-600">
                          ...
                          <span className="text-pink-700 font-medium">{formatPrice(Math.ceil(preOrderTotal * 0.5))}</span>
                        </div>
                    ...
                    <div className="space-y-3">
                      {preOrderItems.map((item) => {
                        ...
                        return (
                          <div
                            key={itemKey}
                            className="flex gap-3 bg-pink-50 rounded-xl p-3 border border-pink-200"
                          >
                            {/* Image */}
                            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 relative">
                              ...
                              <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                                <span className="bg-pink-600 text-white text-xs px-2 py-0.5 rounded-full font-medium">
                                  Pre-pedido
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="font-medium text-gray-900 text-sm line-clamp-1">
                                {item.product.name}
                                {item.variant && (
                                  <span className="text-pink-800 font-normal ml-1">
                                    ({item.variant.name})
                                  </span>
                                )}
                              </h3>
                              ...
                              <p className="text-pink-700 text-xs mt-1">
                                Entrega en ~{settings.preorder_delivery_time}
                              </p>
                              ...
                              <div className="flex items-center justify-between mt-2">
                                <div className="flex items-center gap-1">
                                  <button
                                    onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.variant?.id)}
                                    className="w-8 h-8 rounded-full bg-white border border-pink-200 flex items-center justify-center active:bg-pink-100 transition-colors touch-target"
                                  >
                                    <Minus className="w-3.5 h-3.5 text-gray-600" />
                                  </button>
                                  <span className="w-7 text-center text-sm font-medium">
              ...
              <div className="sticky bottom-0 bg-white border-t border-gray-100 p-4 space-y-3 safe-bottom">
                ...
                {hasPreOrderItems ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between bg-pink-50 rounded-lg px-3 py-2">
                      <span className="text-pink-800 font-medium">Pagas ahora</span>
                      <span className="text-xl font-bold text-pink-700">{formatPrice(advancePayment)}</span>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 bg-[#b55ca6] text-white py-3.5 rounded-xl font-semibold active:bg-[#9c4a8f] active:scale-[0.98] transition-all touch-target"
                  >
