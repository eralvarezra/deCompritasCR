...
function CatalogContent() {
  ...
  const categoriesWithProducts = categories.filter(category => {
    if (categoryIdsWithProducts.has(category.id)) return true
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center mb-3">
            <a href="/" className="flex items-center">
              <img src="/logo.png" alt="DeCompritas" className="h-24 sm:h-32 md:h-36 lg:h-40 w-auto" />
            </a>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-200 focus:border-[#b55ca6] outline-none transition-colors text-base"
            />
          </div>
        </div>
            ...
            <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2">
              <button
                ...
                className={clsx(
                  ...
                  selectedMainCategory === 'all' ? 'bg-[#b55ca6] text-white' : 'bg-gray-100 text-gray-700 active:bg-gray-200'
                )}
              >
                Todos ({productsCount})
              </button>
              {allCategories.map((category) => {
                ...
                const subcategoriesWithProducts = category.subcategories?.filter(sub => categoryIdsWithProducts.has(sub.id)) || []
                  ...
                  <div key={category.id} className="flex-shrink-0">
                    <button
                      onClick={() => {
                        ...
                        if (!isPromociones) toggleCategory(category.id)
                      }}
                      className={clsx(
                        'flex items-center gap-1 px-4 py-2 rounded-full text-sm font-medium transition-colors touch-target',
                        isSelected
                          ? (isPromociones ? 'bg-red-500 text-white' : 'bg-[#b55ca6] text-white')
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide pt-1">
                  <button
                    ...
                    className={clsx(
                      ...
                      selectedSubcategory === 'all' ? 'bg-[#a04792] text-white' : 'bg-gray-50 text-gray-600 active:bg-gray-100'
                    )}
                  >
                    Todos
                  </button>
                  {subcats.map((subcat) => (
                    <button
                      ...
                      className={clsx(
                        'flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors touch-target',
                        selectedSubcategory === subcat.id ? 'bg-[#a04792] text-white' : 'bg-gray-50 text-gray-600 active:bg-gray-100'
                      )}
                    >
                      {subcat.name}
                    </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 pb-28">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <Loader2 className="w-10 h-10 animate-spin text-[#b55ca6]" />
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-500 text-center px-4">Los productos aparecerán aquí cuando estén disponibles.</p>
          </div>
        ) : (
          <>
            {searchQuery && filteredProducts.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <button onClick={() => { setSelectedMainCategory('all'); setSelectedSubcategory('all') }} className="mt-2 text-[#a04792] font-medium">
              ...
              <>
                <p className="text-sm text-gray-500 mb-3 px-1">
                  ...
                  {selectedMainCategory !== 'all' && (
                    <span className="text-[#a04792]">
                      {' '}en {selectedMainCategory === 'promociones' ? 'Promociones' : categoriesWithProducts.find(c => c.id === selectedMainCategory)?.name}
